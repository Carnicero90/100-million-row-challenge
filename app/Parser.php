...
final class Parser
{
    ...
    public function parse(string $inputPath, string $outputPath): void
    {
        ...
        for ($w = 0; $w < self::WORKERS - 1; $w++) {
            ...
            $childPid = pcntl_fork();
            if ($childPid === 0) {
                fwrite(fopen($shmFile, 'wb'), pack('v*', ...self::parseChunk($inputPath, $bounds[$w], $bounds[$w + 1], $dates, $size, $dateCount, $pathsByEntry)));
                exit(0);
            }
            $pids[] = $childPid;
        }

        $merged = self::parseChunk($inputPath, $bounds[$w], $bounds[$w + 1], $dates, $size, $dateCount, $pathsByEntry);
        ...
        for ($i = $dateCount; $i < $size; $i+=$dateCount) {
    ...
    private static function parseChunk($in, $start, $end, $dates, $size, $dateCount, $pathsByEntry) {
        ...
        readChunk:
        if ($remaining > static::BUFFER_SIZE) {
        ...
        while ($pos < $lastLineBreak) {
            $tPos = strpos($chunk, 'T', $pos);
            $counts[$pathsByEntry[(match ($tPos - $pos - 11) {
                4 => 183, 36 => 55, 42 => 59, 44 => 129, 47 => 219,
                9 => match ($chunk[$pos]) { 'p' => 121, 's' => 202 },
                40 => match ($chunk[$pos + 7]) { 'd' => 67, 'i' => 238 },
                45 => match ($chunk[$pos]) { 'c' => 82, 'a' => 130 },
                ...
                30 => match ($chunk[$pos + 29]) { 's' => 10, 'm' => 45, '8' => 93, 'g' => 102, 'k' => 147, '2' => 156, 'e' => 159, 'n' => 189, '3' => 193, '4' => 216, '5' => 241 },
            })] * $dateCount + $dates[substr($chunk, $tPos - 7, 7)]]++;
            $pos = $tPos + 41;
        }
        goto readChunk;
        done:
